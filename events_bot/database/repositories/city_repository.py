...
class CityRepository:
    ...
    async def get_all_active(db: AsyncSession) -> List[City]:
        """Получить все активные города (сохранено для обратной совместимости)"""
        return await CityRepository.get_all_active_ordered_by_id(db)

    @staticmethod
    async def get_all_active_ordered_by_id(db: AsyncSession) -> List[City]:
    ...
    async def get_by_ids(db: AsyncSession, city_ids: List[int]) -> List[City]:
        ...
        result = await db.execute(
            ...
            .order_by(City.id.asc())  # опционально: сохранить порядок
        )
        return list(result.scalars().all())
